...
/*
 * id là mã category dùng trên trang tìm kiếm nâng cao.
 */
const CATEGORY_CONFIG = {
  kids: {
    label: "Nội dung học sinh",
  },
...
/*
 * Các nhóm nhỏ hiển thị ở trang chủ học sinh.
 * Thứ tự ở đây cũng là thứ tự ưu tiên.
 */
const KIDS_SECTIONS = [
  "comedy",
  "schoolLife",
...
/*
 * Các nhóm được phép mở khi bật Nội dung học sinh.
 */
...
/*
 * Đổi phiên bản để không sử dụng cache sai của code cũ.
 */
const CACHE_VERSION = "category-v6-distinct-groups";
...
function isSafeStory(item) {
  ...
  const hasUnsafeGenre =
    UNSAFE_GENRES.some((unsafeGenre) =>
      genres.some(
        (genre) =>
...
/*
 * So sánh mức trùng nhau giữa hai danh sách.
 */
...
function extractItems(html, base) {
  ...
  /*
   * Dự phòng nếu cấu trúc thẻ truyện thay đổi.
   */
  if (items.length < 4) {
    $(
      'a[href*="/truyen-tranh/"]'
...
function categoryCandidateUrls(
  ...
  const urls = [
    /*
     * Ưu tiên tìm kiếm nâng cao có mã category.
     */
    `${base}/tim-kiem-nang-cao/` +
      `trang-${currentPage}.html?` +
      advancedQuery,

    `${base}/tim-kiem-nang-cao.html?` +
      `${advancedQuery}&page=${currentPage}`,
  ...
  /*
   * Thêm đường dẫn thể loại làm phương án dự phòng.
   */
  if (config.slug) {
    if (currentPage === 1) {
      urls.push(
...
/*
 * Không chấp nhận trang bị chuyển về trang chủ hoặc danh sách chung.
 */
...
/*
 * Kiểm tra HTML có dấu hiệu đúng category.
 *
 * Đây là kiểm tra bổ sung. Không bắt buộc trang phải hiển thị
 * tên thể loại, vì cấu trúc TruyenQQ có thể thay đổi.
 */
  ...
  for (const base of BASES) {
    ...
    for (const candidateUrl of candidates) {
      try {
        ...
        /*
         * Nếu URL cuối bị đổi sang trang chủ hoặc trang chung,
         * tuyệt đối không nhận danh sách.
         */
        if (
        ...
        /*
         * Với URL tìm kiếm nâng cao, category phải còn nguyên.
         */
        if (
          /tim-kiem-nang-cao/i.test(
            result.finalUrl
...
async function sectionsAction(
  ...
  const successful = loadedResults
    .filter(
      (entry) =>
        entry.status ===
        "fulfilled"
  ...
  /*
   * Danh sách dấu vân tay đã sử dụng.
   * Nếu hai nhóm trả về cùng danh sách nguồn,
   * chỉ giữ nhóm xuất hiện trước.
   */
  const acceptedFingerprints = [];

  /*
   * URL đã xuất hiện trong các nhóm trước.
   * Một truyện chỉ xuất hiện ở một nhóm trên trang chủ.
   */
  const usedStoryUrls =
    new Set();
  ...
  for (const section of successful) {
    ...
    /*
     * Nguồn trả cùng danh sách cho hai category:
     * bỏ nhóm bị trùng, không hiển thị sai.
     */
    if (duplicatedSource) {
      console.warn(
        "[truyenqq] Bỏ nhóm trùng nguồn:",
        section.key,
        section.sourceUrl
      );
    ...
    /*
     * Không hiển thị nhóm quá ít dữ liệu.
     */
...
async function listAction(
  ...
  /*
   * Tắt Nội dung học sinh và chọn Tất cả.
   */
  if (
    !safeMode &&
    category === "all"
   ...
   * Trang chủ nên gọi action=sections.
   */
  if (category === "kids") {
    const results =
      await mapLimit(
...
async function chapterAction(
  ...
  /*
   * Tìm URL ảnh trong JavaScript nhúng.
   */
  if (!pages.length) {
    const urlPattern =
