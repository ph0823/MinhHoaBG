...
function allowedStoryUrl(rawUrl) {
  try {
    ...
    return ["truyenqqko.com", "truyenqqto.com", "truyenqq.net"].some((domain) => host === domain || host.endsWith(`.${domain}`));
  } catch { return false; }
}
...
function extractChapterNumber(item) {
  const text = clean([item.latestChapter, item.status].filter(Boolean).join(" "));
  const patterns = [
...
function looksVietnameseTitle(title = "") {
  ...
  if (/[ăâđêôơưĂÂĐÊÔƠƯáàảãạấầẩẫậắằẳẵặéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵ]/.test(value)) return true;
  
  const normalized = normalizeText(value);
  const vietnameseWords = ["cua", "toi", "ban", "chung ta", "the gioi", "vuong quoc", "phieu luu", "phep thuat", "hoc vien", "gia dinh", "ngay mai", "dua tre", "co be", "cau be", "chien binh", "anh hung", "cuoc song", "bi mat", "hanh trinh", "ngoi truong", "thieu nien"];
  return vietnameseWords.some((word) => normalized.includes(word));
}

function isCompletedStory(item) {
  const text = normalizeText([item.status, item.latestChapter].filter(Boolean).join(" "));
  return ["hoan thanh", "da hoan thanh", "completed", "complete", "full", "ket thuc", "end"].some((keyword) => text.includes(keyword));
}
...
function recommendationScore(item, category = "") {
  ...
  if (looksVietnameseTitle(item.title)) score += 100;
  if (isCompletedStory(item)) score += 85;

  if (chapterNumber > 10) {
    score += 60;
    score += Math.min(25, Math.floor(chapterNumber / 10));
  } else {
    score += Math.min(20, chapterNumber);
  }
  ...
  if (sourceCategories.length > 1) {
    score += Math.min(15, (sourceCategories.length - 1) * 4);
  }
  return score;
}
...
function decorateRecommendation(item, category = "") {
  return {
    ...
    vietnameseTitle: looksVietnameseTitle(item.title),
    recommendationScore: recommendationScore(item, category),
  };
}

function sortRecommendedStories(items, category = "") {
  return [...items].map((item) => decorateRecommendation(item, category)).sort((a, b) => {
    const scoreDifference = b.recommendationScore - a.recommendationScore;
    if (scoreDifference !== 0) return scoreDifference;
    const vietnameseDifference = Number(b.vietnameseTitle) - Number(a.vietnameseTitle);
    if (vietnameseDifference !== 0) return vietnameseDifference;
    const completedDifference = Number(b.completed) - Number(a.completed);
    if (completedDifference !== 0) return completedDifference;
    const bOverTen = b.chapterNumber > 10 ? 1 : 0;
    const aOverTen = a.chapterNumber > 10 ? 1 : 0;
    if (bOverTen !== aOverTen) return bOverTen - aOverTen;
    if (b.chapterNumber !== a.chapterNumber) return b.chapterNumber - a.chapterNumber;
    return clean(a.title).localeCompare(clean(b.title), "vi", { sensitivity: "base" });
  });
}
...
async function fetchHtml(url, timeout = 8000) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      const response = await fetch(url, { headers: HTML_HEADERS, redirect: "follow", signal: controller.signal });
...
async function fetchFromBases(pathBuilder) {
  ...
  for (const base of BASES) {
    try { return await fetchHtml(pathBuilder(base)); } catch (error) { lastError = error; }
  }
...
function extractImageSource(image) {
  ...
  return image.attr("data-original") || image.attr("data-cfsrc") || image.attr("data-src") || image.attr("data-lazy-src") || image.attr("data-url") || image.attr("data-image") || image.attr("data-echo") || firstSrcset || image.attr("src") || "";
}

function extractGenres(box, $) {
  const genres = [];
  box.find([".list-tags a", ".genres a", ".genre a", '.book_info a[href*="the-loai"]', 'a[href*="/the-loai/"]'].join(","))
    .each((_, element) => {
...
function extractItems($, base) {
  const items = [];
  const selectors = [".book_avatar", ".book_item", ".story-item", ".item-manga", ".list_grid .item", ".comic-item", ".item"];
  ...
  $(selectors.join(",")).each((_, element) => {
    ...
    const title = clean(storyLink.attr("title") || box.find(".book_name, .title, .name, h2, h3").first().text() || storyLink.text());
    if (!url || !title || isChapterUrl(url)) return;

    const chapterElement = box.find(['a[href*="-chap-"]', 'a[href*="/chap-"]', 'a[href*="/chapter-"]', 'a[href*="/chuong-"]', ".chapter", ".last_chapter", ".chapter-name"].join(",")).first();
    ...
    items.push({
      ...
      status: clean(box.find(".status, .book_status, .story-status").first().text()),
  ...
  if (items.length < 4) {
    $('a[href*="/truyen-tranh/"]').each((_, element) => {
      ...
      const parent = link.closest(["li", "article", ".item", ".book_item", ".story-item"].join(","));
      const title = clean(link.attr("title") || parent.find(".book_name, .title, .name, h2, h3").first().text() || link.text());
      if (!title || /^chapter\s*[\d.]+$/i.test(title)) return;

      const chapterElement = parent.find(['a[href*="-chap-"]', 'a[href*="/chap-"]', 'a[href*="/chapter-"]', 'a[href*="/chuong-"]'].join(",")).first();
      ...
      items.push({
        ...
        cover: absolute(extractImageSource(image), base),
        latestChapter: clean(chapterElement.attr("title") || chapterElement.text()),
        status: clean(parent.find(".status, .book_status, .story-status").first().text()),
...
function getTotalPages($, currentPage = 1) {
  ...
  $(".pagination a, .page_redirect a, a[href*='page='], a[href*='trang-']").each((_, element) => {
    ...
    maximumPage = Math.max(maximumPage, textPage, queryMatch ? Number(queryMatch[1]) : 0, pathMatch ? Number(pathMatch[1]) : 0);
  });
  return maximumPage;
}
...
function categoryCandidateUrls(base, config, page) {
  ...
  if (config.slug) {
    if (currentPage === 1) urls.push(`${base}/the-loai/${config.slug}-${config.id}`, `${base}/the-loai/${config.slug}-${config.id}.html`, `${base}/the-loai/${config.slug}`, `${base}/the-loai/${config.slug}.html`);
    else urls.push(`${base}/the-loai/${config.slug}-${config.id}/trang-${currentPage}.html`, `${base}/the-loai/${config.slug}-${config.id}.html?page=${currentPage}`, `${base}/the-loai/${config.slug}/trang-${currentPage}.html`);
  }
  return [...new Set(urls)];
}
...
function categoryResponseLooksValid(finalUrl, config) {
  try {
    ...
    return pathname.includes(`/the-loai/${config.slug}`);
  } catch { return false; }
}

function categoryHtmlLooksValid(html, config) {
  const normalizedHtml = normalizeText(html);
  return normalizedHtml.includes(`category=${config.id}`) || normalizedHtml.includes(normalizeText(config.label)) || normalizedHtml.includes(normalizeText(config.slug.replace(/-/g, " ")));
}
  ...
  for (const base of BASES) {
    ...
    for (const candidateUrl of candidates) {
      try {
        ...
        const value = { items, totalPages: getTotalPages($, currentPage), sourceUrl: result.finalUrl, fingerprint: storyListFingerprint(items) };
...
async function homeKidsAction(limitText = String(HOME_KIDS_LIMIT)) {
  const limit = Math.min(HOME_KIDS_LIMIT, Math.max(12, Number.parseInt(limitText, 10) || HOME_KIDS_LIMIT));
  const cacheKey = `${CACHE_VERSION}:home-kids:${limit}`;
  const cached = cacheGet(cacheKey);
  if (cached) return cached;

  const jobs = HOME_KIDS_CATEGORIES.flatMap((categoryKey) => [1, 2].map((page) => ({ categoryKey, page })));
  const loadedResults = await mapLimit(jobs, 3, async ({ categoryKey, page }) => {
    const result = await loadCategoryByKey(categoryKey, page);
    return { categoryKey, items: result.items };
  });

  const byStory = new Map();
  for (const result of loadedResults) {
    if (result.status !== "fulfilled") continue;
    const { categoryKey, items } = result.value;
    for (const rawItem of items) {
      if (!isSafeStory(rawItem)) continue;
      const identity = storyIdentity(rawItem);
      if (!identity) continue;
      
      const existing = byStory.get(identity);
      if (existing) {
        existing.sourceCategories = [...new Set([...(existing.sourceCategories || []), categoryKey])];
        if (!existing.cover && rawItem.cover) existing.cover = rawItem.cover;
        if (!existing.latestChapter && rawItem.latestChapter) existing.latestChapter = rawItem.latestChapter;
        if (!existing.status && rawItem.status) existing.status = rawItem.status;
        if ((!existing.genres || !existing.genres.length) && rawItem.genres?.length) existing.genres = rawItem.genres;
      } else {
        byStory.set(identity, { ...rawItem, sourceCategories: [categoryKey] });
      }
    }
  }

  const items = sortRecommendedStories([...byStory.values()], "kids").slice(0, limit);
  if (!items.length) throw new Error("Không tải được truyện thiếu nhi phù hợp cho màn hình chính.");

  return cacheSet(cacheKey, {
    items, page: 1, totalPages: 1, category: "kids", categoryLabel: CATEGORY_CONFIG.kids.label,
    safeMode: true, homeRecommendations: true, generatedAt: Date.now(), categories: CATEGORY_CONFIG,
  });
}
...
async function sectionsAction(limitText = "6") {
  ...
  const loadedResults = await mapLimit(KIDS_SECTIONS, 3, async (categoryKey) => {
    ...
    const safeItems = result.items.filter(isSafeStory);
    return { key: categoryKey, label: CATEGORY_CONFIG[categoryKey].label, items: sortRecommendedStories(safeItems, categoryKey), totalPages: result.totalPages, sourceUrl: result.sourceUrl, fingerprint: result.fingerprint };
  });
  ...
  for (const section of successful) {
    const duplicatedSource = acceptedFingerprints.some((previous) => previous.fingerprint === section.fingerprint || listSimilarity(previous.items, section.items) >= 0.75);
...
async function loadLatestPage(page) {
  const currentPage = Math.max(1, Number(page) || 1);
  let result;
  try { result = await fetchFromBases((base) => `${base}/truyen-moi-cap-nhat/trang-${currentPage}.html`); } 
  catch { result = await fetchFromBases((base) => `${base}/?page=${currentPage}`); }
...
async function listAction(pageText, queryText, categoryText = "kids", safeText = "1") {
  ...
  if (query) {
    ...
    items = sortRecommendedStories(items, category);
    return { items, page: currentPage, totalPages: getTotalPages($, currentPage), category, safeMode, categories: CATEGORY_CONFIG };
  }

  if (!safeMode && category === "all") {
    const result = await loadLatestPage(currentPage);
    return { ...result, items: sortRecommendedStories(result.items, "all"), page: currentPage, category, safeMode, categories: CATEGORY_CONFIG };
  }

  if (category === "kids") return homeKidsAction(HOME_KIDS_LIMIT);
...
module.exports = async function handler(request, response) {
  if (request.method !== "GET") { response.setHeader("Allow", "GET"); return sendJson(response, 405, { error: "Chỉ hỗ trợ phương thức GET" }); }
  const action = clean(request.query?.action || "list").toLowerCase();

  try {
    if (action === "sections") { const result = await sectionsAction(request.query?.limit); return sendJson(response, 200, result, "public, s-maxage=300, stale-while-revalidate=600"); }
    if (action === "list") { const result = await listAction(request.query?.page, request.query?.q, request.query?.category, request.query?.safe); return sendJson(response, 200, result, "public, s-maxage=180, stale-while-revalidate=300"); }
    if (action === "detail") { const result = await detailAction(request.query?.url); return sendJson(response, 200, result, "public, s-maxage=300, stale-while-revalidate=600"); }
    if (action === "chapter") { const result = await chapterAction(request.query?.url); return sendJson(response, 200, result, "public, s-maxage=300, stale-while-revalidate=600"); }
    if (action === "image") return await imageAction(request.query?.url, request.query?.ref, response);
